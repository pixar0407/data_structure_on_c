...
void func(char *list, char* ans,int *length, int *depth) { //length(초기 str길이,상수)와 depth(front합값)는 call by ref.
	char front[101];
	char back[101];
	char temp[101];
	...
	for (i = 1; i <= len_list; i++) { //여기서 i는 front의 길이로 update되어, (i)for 안에서는 상수처럼 쓰인다.
		...
		//recursion이 끝나고 돌아왔을 때, ans를 초기화 시켜주기 때문에 구버전 ans를 temp에 임시로 저장한다.
		temp[0] = '\0';
		for (j = 0; j <= 101; j++) {
			if (ans[j] == NULL)
				break;
		...
		*depth += i; //ans에 front를 추가한 만큼 depth를 증가시킨다.
		...
		else {//ans가 가득 찼다면, 마지막 '/'추가하지 않고, 바로 출력을 주며, recursion을 종료한다.
			printf("%s\n", ans);
			ans[0] = '\0';
			*(depth) = 0;
		}
